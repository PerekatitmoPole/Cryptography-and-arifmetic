...
\textbf{Теорема Эйлера.} Если $a\in(\Z/n\Z)^{\ast}$ (то есть $\gcdop(a,n)=1$), то
\[
...
\begin{itemize}
  \item $\Z/n\Z$ — кольцо.
  \item $(\Z/n\Z)^{\ast}$ — группа обратимых элементов.
  \item $\varphi(n)$ — число элементов $x\in\{1,\dots,n-1\}$, взаимно простых с $n$.
\end{itemize}

\textbf{Замечание.} $\varphi(n)=|(\Z/n\Z)^{\ast}|$.

\textbf{Критерий обратимости.} Для $m\in\Z/n\Z$:
\[
m\in(\Z/n\Z)^{\ast} \iff \gcdop(m,n)=1.
\]
Идея: $\gcdop(m,n)=1 \Rightarrow \exists x,y\in\Z: mx+ny=1 \Rightarrow mx\equiv 1\pmod n$.
...
Пусть $x_1,\dots,x_{\varphi(n)}$ — все элементы $(\Z/n\Z)^{\ast}$.
Тогда $ax_1,\dots,ax_{\varphi(n)}$ — просто перестановка $(\Z/n\Z)^{\ast}$, значит
\[
...
Если $a\in(\Z/n\Z)^{\ast}$, то наименьшее $k\in\N$, для которого $a^k\equiv 1\pmod n$,
...
(\Z/3\Z)^{\ast}=\{1,2\},\quad
(\Z/4\Z)^{\ast}=\{1,3\},\quad
(\Z/5\Z)^{\ast}=\{1,2,3,4\},\quad
(\Z/6\Z)^{\ast}=\{1,5\}.
\]
\[
(\Z/7\Z)^{\ast}\cong \Z_6,\qquad
...
Пусть $a\in(\Z/n\Z)^{\ast}$ и $k=\ord(a)$, то есть $a^k\equiv 1\pmod n$.
Тогда:
\[
...
Пусть $m_1,\dots,m_r\in\N$ попарно взаимно просты.
Для любых $a_1,\dots,a_r\in\Z$ система
\[
\begin{cases}
...
Тогда $\gcdop(m_i',m_i)=1$, поэтому $\exists \alpha_i\in\Z/m_i\Z$:
\[
...
Пусть $K=\F_q$ — конечное поле из $q=p^s$ элементов.
Факт: $K^{\ast}$ — циклическая группа порядка $(q-1)$.

\textbf{Лемма.} Для $u\in\N$ положим
\[
S(u)=\sum_{x\in K} x^u.
\]
Тогда
...
\[
V=\{x\in K^n \st f_1(x)=\cdots=f_m(x)=0\}.
\]
Тогда
\[
|V|\equiv 0\pmod p.
...
\[
m\in \Z/n\Z,\qquad E(m)=m^e\pmod n,\qquad D(c)=c^d\pmod n.
\]
...
\textbf{Теорема Люка.}
Если найдётся $a\in(\Z/n\Z)^{\ast}$ такое, что
\[
...
Для $a\in(\Z/n\Z)^{\ast}$ верно $a^{\varphi(n)}\equiv 1$.
Пусть $d=\gcdop(n-1,\varphi(n))$, тогда $a^d\equiv 1$, и это приводит к противоречию
с условием $a^{\frac{n-1}{p}}\not\equiv 1$.

\textbf{Числа Кармайкла.}
Определение: $m\in\N$ — число Кармайкла, если $m$ составное и
\[
a^m \equiv a \pmod m \quad \forall a\in\Z.
\]
